...
export default async function handler(
  ...
  try {
    ...
    const existingData = await query('SELECT COUNT(*) as count FROM token_usage');
    ...
    return res.status(200).json({ 
      message: 'Token usage table and views created successfully',
      sampleDataAdded: existingData[0].count === 0
    });
    
  } catch (error) {
    console.error('Error setting up token usage table:', error);
    return res.status(500).json({ 
      message: 'Failed to setup token usage table',
